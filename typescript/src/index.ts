...
function representFontFamilyTokenValue(value: FontTokenValue, allTokens: Array<Token>, allGroups: Array<TokenGroup>): any {
  let result: any
  if (value.referencedToken) {
    // Forms reference
    result = referenceWrapper(referenceName(value.referencedToken, allGroups))
...
/** Represent font weight value either as reference or as plain representation */
function representFontWeightTokenValue(value: FontTokenValue, allTokens: Array<Token>, allGroups: Array<TokenGroup>): any {
  let result: any
  if (value.referencedToken) {
    // Forms reference
    result = referenceWrapper(referenceName(value.referencedToken, allGroups))
...
function tokenWrapper(token: Token, value: any) {
  return {
    ...
    "$extensions": {
      "org.supernova-io": {
        "id": token.id,
        "figma-style-key": token.origin ? token.origin.id?.slice(2,-1) : undefined,
